...
function defaultProxy(){ return 'https://corsproxy.io/?url='; }
function withProxy(url, forceDefault=false){
  const user = (CFG().proxy||'').trim();
  const p = user || (forceDefault ? defaultProxy() : '');
  if(!p) return url;
  // Query-string style proxy (e.g. corsproxy.io/?url= or allorigins.win/raw?url=)
  if(p.includes('?url=') || p.includes('&url=')) return p + encodeURIComponent(url);
  // Prefix style (e.g. cors.isomorphic-git.org/)
...
async function fetchFavoriot(url, headers, {attempt=0, viaDefaultProxy=false}={}){
  // If a proxy is configured, use it first — direct requests fail with CORS from file://
  const hasProxy = !!(CFG().proxy||'').trim();
  const usedUrl = (hasProxy && attempt===0) ? withProxy(url) : (viaDefaultProxy ? withProxy(url, true) : url);
  ...
  try{
    ...
    clearTimeout(t);
    // Fallback chain: proxy-first → direct → default proxy
    if(attempt===0 && hasProxy){ return fetchFavoriot(url, headers, {attempt:1, viaDefaultProxy:false}); }
    if(attempt===1 && !viaDefaultProxy){ return fetchFavoriot(url, headers, {attempt:2, viaDefaultProxy:true}); }
    throw e;
  }
}
...
(function(){
  ...
  const KNOWN = ['https://corsproxy.io/?url=','https://api.allorigins.win/raw?url=','https://cors.isomorphic-git.org/',''];
