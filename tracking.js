...
function trilatLM(beacons, initXY){
  ...
  // ---------------------------------------------------------------
  // STEP 1: Linear Least Squares (closed-form)
  // Subtracts pairs of circle equations to eliminate quadratic terms,
  // giving a linear system Ax = b. Solves for the best-fit point
  // even when circles don't intersect at all.
  // ---------------------------------------------------------------
  function linearLS(bcs){
    // Use first beacon as reference, subtract its equation from all others
    const ref = bcs[0];
    const A = [], b = [];
    for(let i = 1; i < bcs.length; i++){
      const bi = bcs[i];
      // (xi^2 - x0^2) + (yi^2 - y0^2) - (di^2 - d0^2) = 2(xi-x0)x + 2(yi-y0)y
      const ax = 2*(bi.x - ref.x);
      const ay = 2*(bi.y - ref.y);
      const bv = (bi.x*bi.x - ref.x*ref.x)
               + (bi.y*bi.y - ref.y*ref.y)
  ...
  // ---------------------------------------------------------------
  // STEP 2: LM refinement starting from the LS solution
  // Polishes the result to handle any nonlinear residual.
  // ---------------------------------------------------------------
  function lmRefine(sx, sy, bcs){
    let x=sx, y=sy, lambda=1e-3;
    ...
    for(let it=0; it<maxIter; it++){
      ...
      for(const b of bcs){
        ...
        let r=Math.hypot(dx,dy); if(r<1e-9) r=1e-9;
        const ri=r-b.d;
      ...
      prevCost=cost;
      const a11=j11+lambda, a22=j22+lambda;
      const det=a11*a22-j12*j12;
      if(Math.abs(det)<1e-14){ lambda*=10; continue; }
      const d1=(-g1*a22+g2*j12)/det;
      const d2=(-g2*a11+g1*j12)/det;
      const nx=x+d1, ny=y+d2;
      let newCost=0;
      for(const b of bcs){
        const dx=nx-b.x, dy=ny-b.y; let r=Math.hypot(dx,dy); if(r<1e-9) r=1e-9;
        newCost+=(r-b.d)*(r-b.d);
      }
      if(newCost<cost){ x=nx; y=ny; lambda=Math.max(lambda/5,1e-9); if(Math.hypot(d1,d2)<eps) break; }
      else { lambda=Math.min(lambda*5,1e8); }
    }
    let se=0; for(const b of bcs){ const e=Math.hypot(x-b.x,y-b.y)-b.d; se+=e*e; }
    return { x, y, cost:se };
  }

  // ---------------------------------------------------------------
  // STEP 3: Outlier rejection — if RMS is high, try dropping one
  // beacon at a time and keep the subset with the lowest RMS.
  // Only kicks in when beacons.length > 3 and RMS is poor.
  // ---------------------------------------------------------------
  function costOf(bcs, x, y){ let s=0; for(const b of bcs){ const e=Math.hypot(x-b.x,y-b.y)-b.d; s+=e*e; } return s; }

  // Primary solve: Linear LS → LM refinement
  function solve(bcs){
    const ls = linearLS(bcs);
    const sx = ls ? ls[0] : bcs.reduce((s,b)=>s+b.x,0)/bcs.length;
    const sy = ls ? ls[1] : bcs.reduce((s,b)=>s+b.y,0)/bcs.length;
    return lmRefine(sx, sy, bcs);
  }

  let best = solve(beacons);

  // If more than 3 beacons and RMS looks high, try dropping each beacon once
  if(beacons.length > 3){
    const rmsThreshold = 2.0; // metres — tune if needed
    const rms = Math.sqrt(best.cost / beacons.length);
    if(rms > rmsThreshold){
      for(let i=0; i<beacons.length; i++){
        const subset = beacons.filter((_,j)=>j!==i);
        const res = solve(subset);
        const subRms = Math.sqrt(res.cost / subset.length);
        if(subRms < Math.sqrt(best.cost / beacons.length)){
          best = res;
        }
      }
    }
