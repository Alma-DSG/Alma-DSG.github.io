...
(function(){
  ...
  window.lastRxByDev = window.lastRxByDev || new Map();
  ...
  if (typeof window.fetchOneDeviceLatest === 'function' && !window.__wrappedFetchOneDeviceLatest){
    ...
    window.fetchOneDeviceLatest = async function(dev, idx){
      ...
      try{
        ...
        // A0) Receipt marker: keep Online based on browser receiving data even if stream time doesn't move
        try{ if (window.lastRxByDev) window.lastRxByDev.set(normId, Date.now()); }catch(_){ }
  ...
  if (typeof window.computeStatus === 'function' && !window.__overrodeComputeStatus){
    ...
    window.computeStatus = function(normId){
      // Prefer receipt time (browser got fresh data) to avoid falling into Sleep when backend timestamps don't advance
      let base = undefined;
      try{
        const rx = (window.lastRxByDev && window.lastRxByDev.get(normId)) ?? undefined;
        if (Number.isFinite(rx)) base = rx;
      }catch(_){}
      if (!Number.isFinite(base)){
        const last = (typeof lastSeenByDev!=='undefined') ? lastSeenByDev.get(normId) : undefined;
        if (Number.isFinite(last)) base = last;
      }
      const delta = Number.isFinite(base) ? (Date.now()-base) : Infinity;
      ...
      const defaultGrace = (typeof SLEEP_GRACE_MS!=='undefined' ? SLEEP_GRACE_MS : 20000);
      let refreshMs = 8000;
      try{ if (typeof CFG==='function') refreshMs = (CFG().refreshMs||refreshMs); }catch(_){}
      const minGraceMs = Math.max(20000, refreshMs*2);

      let graceMs = (activeSec && Number.isFinite(activeSec) && activeSec>0) ? activeSec*1000 : defaultGrace;
      if (!Number.isFinite(graceMs) || graceMs <= 0) graceMs = defaultGrace;
      graceMs = Math.max(graceMs, minGraceMs);
