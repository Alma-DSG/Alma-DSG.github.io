...
const lastStreamTsByDev=new Map();
const lastSigByDev=new Map();
const lastArrivedByDev=new Map();
const avgIntervalByDev=new Map(); // EMA of inter-arrival (ms)
...
function computeStatus(normId){
  ...
  // Grace window before we call it "Sleep".
  // - At least 20s
  // - At least 2x the dashboard refresh
  // - If we have observed device update interval, use ~1.5x that (capped by sleepMs)
  let graceMs = Math.max(SLEEP_GRACE_MS, (CFG()?.refreshMs || 8000) * 2);
  const avg = avgIntervalByDev.get(normId);
  if (Number.isFinite(avg) && avg > 0) graceMs = Math.max(graceMs, Math.min(avg * 1.5, sleepMs));
  graceMs = Math.min(graceMs, sleepMs);

  const online = Number.isFinite(delta) && delta <= graceMs;
  const sleeping = !online && (sleepSec!=null) && Number.isFinite(delta) && delta <= sleepMs;
  const offline = !Number.isFinite(delta) || delta > sleepMs;
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    // Keep server-provided timestamp (if any) for display/reasoning
    const ts = time ? Date.parse(time) : NaN;
    if(Number.isFinite(ts)) lastStreamTsByDev.set(normId, ts); else lastStreamTsByDev.delete(normId);

    // Status should reflect when we actually receive a NEW record.
    // (Sometimes Favoriot records can arrive with identical timestamps.)
    const now = Date.now();
    ...
    if(sig && sig !== lastSigByDev.get(normId)){
      const prev = lastArrivedByDev.get(normId);
      ...
      lastSeenByDev.set(normId, now);
    } else {
      // Seed once if we have nothing yet.
      if(!Number.isFinite(lastSeenByDev.get(normId))){
        if(Number.isFinite(ts)) lastSeenByDev.set(normId, ts);
      }
    }
    ...
    (function(){
      ...
      if(st.offline){
        ...
        if(healthByDev.get(normId)===false){
          ...
          const ts2 = (lastStreamTsByDev.get(normId) ?? lastSeenByDev.get(normId));
          const when = Number.isFinite(ts2) ? new Date(ts2).toLocaleString() : '—';
          if(st.sleepSec){
...
function restoreTriMarkersFromCache(){
  ...
  list.forEach((d,i)=>{
    ...
    if(c.time!=null){ const ts = (typeof c.time==='number') ? c.time : Date.parse(c.time); if(Number.isFinite(ts)) lastSeenByDev.set(norm, ts); }
