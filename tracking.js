  ...
  if (typeof window.fetchOneDeviceLatest === 'function' && !window.__wrappedFetchOneDeviceLatest){
    ...
    window.fetchOneDeviceLatest = async function(dev, idx){
      ...
      try{
        ...
        // A1) Always stamp now as lastSeen after a successful fetch so the device
        //     immediately reads as Online — the server timestamp can be slightly old
        //     (device just woke up) which would otherwise make delta > graceMs and
        //     show Sleep right away. The real server timestamp is already stored by
        //     the original fetchOneDeviceLatest; we overwrite it here with wall-clock
        //     now so computeStatus sees delta ≈ 0 right after new data arrives.
        try{
