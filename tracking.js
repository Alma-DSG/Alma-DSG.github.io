...
const lastFetchSuccessAt=new Map(); // tracks wall-clock time of last successful data fetch
...
function computeStatus(normId){
  ...
  // If we just received a successful fetch response within SLEEP_GRACE_MS,
  // always show Online first — regardless of the server timestamp age.
  const lastFetch = lastFetchSuccessAt.get(normId);
  const fetchedRecently = lastFetch && (Date.now() - lastFetch) <= SLEEP_GRACE_MS;
  if(fetchedRecently && enoughBeacons){
    return { online:true, sleeping:false, offline:false, delta, sleepSec };
  }
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    const normId=ensureDevId(dev.id);
    lastFetchSuccessAt.set(normId, Date.now()); // mark that we just received fresh data
...
(function(){
  ...
  if (typeof window.computeStatus === 'function' && !window.__overrodeComputeStatus){
    ...
    window.computeStatus = function(normId){
      ...
      const enoughBeacons = (typeof healthByDev!=='undefined') ? (healthByDev.get(normId)!==false) : true;

      // If we just received a successful fetch response within graceMs, show Online first.
      const lastFetch = (typeof lastFetchSuccessAt!=='undefined') ? lastFetchSuccessAt.get(normId) : undefined;
      const fetchedRecently = lastFetch && (Date.now() - lastFetch) <= graceMs;
      if(fetchedRecently && enoughBeacons){
        return { online:true, sleeping:false, offline:false, delta, sleepSec };
      }
