...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    // Use Date.now() as lastSeen when we receive valid data so that delta≈0
    // when refreshHealthDots() runs — ensuring Online shows first.
    // The server timestamp (ts) can be older than graceMs causing Sleep to show
    // immediately even on fresh data.
    if(Number.isFinite(ts)) lastSeenByDev.set(normId, Date.now()); else lastSeenByDev.set(normId, undefined);
