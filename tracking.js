...
function computeStatus(normId){
  ...
  const online = !offline && !sleeping && Number.isFinite(fetchDelta) && fetchDelta <= SLEEP_GRACE_MS;
...
(function(){
  ...
  if (typeof window.computeStatus === 'function' && !window.__overrodeComputeStatus){
    ...
    window.computeStatus = function(normId){
      ...
      // Online: new data arrived recently (within grace period since last fetch)
      const online   = !offline && !sleeping && Number.isFinite(fetchDelta) && fetchDelta <= graceMs;
