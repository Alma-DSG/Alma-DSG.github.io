...
const lastArrivedByDev=new Map(); // local arrival time of newest record
const lastSigByDev=new Map();      // signature of newest record (for detecting changes)
const avgIntervalByDev=new Map();  // EMA of record inter-arrival (ms)
...
function isStale(normId){
  const a = lastArrivedByDev.get(normId);
  const ts = Number.isFinite(a) ? a : lastSeenByDev.get(normId);
  if(!ts) return true;
  return (Date.now() - ts) >= CONFIG.staleMs;
}
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    // Detect NEW records even when server timestamp repeats; drive Online/Sleep from *arrival* time.
    try{
      const now = Date.now();
      const sig = (function(rec){
        if(!rec) return '';
        const rid = rec.stream_id ?? rec._id ?? rec.id ?? rec.streamId ?? rec.streamid ?? rec.streamID;
        if(rid!=null) return String(rid);
        const t = rec.stream_created_at || rec.created_at || rec.timestamp || '';
        let d=''; try{ d = JSON.stringify(rec.data||{}); }catch(_){}
        return String(t) + '|' + d;
      })(last);
      if(sig){
        const prevSig = lastSigByDev.get(normId);
        if(prevSig !== sig){
          const prevArr = lastArrivedByDev.get(normId);
          if(Number.isFinite(prevArr)){
            const dt = now - prevArr;
            if(dt > 0){
              const prevAvg = avgIntervalByDev.get(normId);
              const alpha = 0.2; // EMA smoothing
              const nextAvg = Number.isFinite(prevAvg) ? (prevAvg*(1-alpha) + dt*alpha) : dt;
              avgIntervalByDev.set(normId, nextAvg);
            }
          }
          lastArrivedByDev.set(normId, now);
          lastSigByDev.set(normId, sig);
        }
      }
    }catch(_){}
...
(function(){
  ...
  if (typeof window.computeStatus === 'function' && !window.__overrodeComputeStatus){
    ...
    window.computeStatus = function(normId){
      // Prefer *arrival time* so status reflects new data even if stream timestamp repeats
      let last = undefined;
      try{
        if (typeof lastArrivedByDev!=='undefined'){
          const a = lastArrivedByDev.get(normId);
          if (Number.isFinite(a)) last = a;
        }
      }catch(_){}
      if (!Number.isFinite(last)){
        last = (typeof lastSeenByDev!=='undefined') ? lastSeenByDev.get(normId) : undefined;
      }
      const delta = Number.isFinite(last) ? Math.max(0, (Date.now()-last)) : Infinity;
      ...
      // Grace window before calling it Sleep:
      // - at least SLEEP_GRACE_MS (default 20s)
      // - at least 2x dashboard refresh
      // - if we observe device interval, use ~1.5x that (capped by sleepMs)
      // - Active_Time_Sec can extend grace, but never *reduce* it
      let graceMs = (typeof SLEEP_GRACE_MS!=='undefined' ? SLEEP_GRACE_MS : 20000);
      try{
        const cfg = (typeof CFG==='function') ? CFG() : null;
        const r = (cfg && Number.isFinite(cfg.refreshMs)) ? cfg.refreshMs : 8000;
        graceMs = Math.max(graceMs, r*2);
      }catch(_){}
      if (Number.isFinite(activeSec) && activeSec>0){
        graceMs = Math.max(graceMs, activeSec*1000);
      }
      try{
        if (typeof avgIntervalByDev!=='undefined'){
          const avg = avgIntervalByDev.get(normId);
          if (Number.isFinite(avg) && avg>0){
            graceMs = Math.max(graceMs, Math.min(avg*1.5, sleepMs));
          }
        }
      }catch(_){}
      graceMs = Math.min(graceMs, sleepMs);
      ...
      return { online, sleeping, offline, delta, sleepSec, graceMs };
    };
  }
