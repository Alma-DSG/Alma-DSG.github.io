...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    if(Number.isFinite(ts)){
      const oldTs = prevServerTs.get(normId);
      if(ts !== oldTs){
        // Server timestamp changed = new data. Reset to now so Online shows immediately.
        prevServerTs.set(normId, ts);
        lastSeenByDev.set(normId, Date.now());
      } else {
        // Same timestamp = no new data. Keep existing lastSeenByDev unchanged.
        if(!lastSeenByDev.has(normId)) lastSeenByDev.set(normId, ts);
      }
    } else {
