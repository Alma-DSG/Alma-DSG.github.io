...
const lastServerTsByDev=new Map(); // tracks previous server timestamp to detect genuinely new data
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    // Only stamp Date.now() when the server timestamp is genuinely new (data changed).
    // If the server timestamp is the same as last fetch, the device hasn't sent new data —
    // keep the old lastSeenByDev so the sleep timer counts correctly from last real data.
    // If it IS new data, stamp now so Online shows immediately before the timer decays.
    if(Number.isFinite(ts)){
      const prevTs = lastServerTsByDev.get(normId);
      if(ts !== prevTs){
        // New data arrived — show Online immediately
        lastServerTsByDev.set(normId, ts);
        lastSeenByDev.set(normId, Date.now());
      } else {
        // Same timestamp — no new data, don't reset the clock
        if(!Number.isFinite(lastSeenByDev.get(normId))) lastSeenByDev.set(normId, ts);
      }
    } else {
