/* One-time migration to remove bundled demo tracker, then never touch user data again */
(function(){
  try{
    if(localStorage.getItem('__alma_migrated_nopreset_v2__') === '1') return;
    const HINTS = /REMOVED_DEMO_ID|Tracker\s*1|almadesign/i;
    // Remove any items containing demo hints
    for (let i = localStorage.length - 1; i >= 0; i--) {
      const k = localStorage.key(i);
      const v = localStorage.getItem(k) || "";
      if (HINTS.test(k) || HINTS.test(v)) {
        try { localStorage.removeItem(k); } catch(_){}
      }
    }
    // Filter structured trackers store if present
    const tRaw = localStorage.getItem('trackers');
    if (tRaw) {
      try {
        const arr = JSON.parse(tRaw);
        if (Array.isArray(arr)) {
          const filtered = arr.filter(x => {
            try { return !HINTS.test(JSON.stringify(x)); } catch(_) { return true; }
          });
          localStorage.setItem('trackers', JSON.stringify(filtered));
        }
      } catch(_){}
    }
    localStorage.setItem('__alma_migrated_nopreset_v2__','1');
  }catch(_){}
})();
...
let map;
try {
  map = L.map('map', {  minZoom:2, maxZoom:22 , crs: L.CRS.EPSG3857, preferCanvas: true, zoomSnap: 1, zoomDelta: 1 }).setView([3.1390, 101.6869], 14);
...
L.control.layers({ 'Streets (OSM)':streetsOSM, 'Satellite (Esri)':esriA },
...
// All known free CORS proxies, tried in order
const CORS_PROXIES = [
  'https://api.allorigins.win/raw?url=',   // query-string style
  'https://api.cors.lol/?url=',             // query-string style
  'https://proxy.corsfix.com/?',            // query-string style
  'https://cors.isomorphic-git.org/',       // prefix style
];
function applyProxy(proxyUrl, targetUrl){
  if(!proxyUrl) return targetUrl;
  if(proxyUrl.includes('?url=') || proxyUrl.includes('/?')) return proxyUrl + encodeURIComponent(targetUrl);
  return proxyUrl.replace(/\/$/,'') + '/' + targetUrl;
}
function getUserProxy(){ return (CFG().proxy||'').trim(); }
function getProxyList(){
  const user = getUserProxy();
  if(user) return [user, ...CORS_PROXIES.filter(p=>p!==user)];
  return CORS_PROXIES;
}
function headersFor(){
  const cfg=CFG();
...
function placeDeviceMarker(dev, idx, lat, lon, time){
  ...
  if(lat!=null&&lon!=null){
    if(!m){ m=L.circleMarker([lat,lon],{renderer: vectorRenderer, radius:7,color:color,weight:3,fillOpacity:.7}); devMarkers.set(dev.id,m); }
    m.setLatLng([lat,lon]);
...
function placeTriMarker(dev, idx, lat, lon, rms){
  ...
  if(lat!=null && lon!=null){
    ...
    if(!m){ m=L.circleMarker([lat,lon],{renderer: vectorRenderer, radius:8,color:triColor,weight:3,fillOpacity:.7}); triMarkers.set(key,m); }
    m.setLatLng([lat,lon]);
...
/* ================= Favoriot fetching ================= */
async function doFetch(url, headers, timeoutMs=12000){
...
async function fetchFavoriot(url, headers){
  const proxies = getProxyList();
  let lastErr;
  // Try each proxy in turn
  for(const proxy of proxies){
    const proxyUrl = applyProxy(proxy, url);
    try{
      const r = await doFetch(proxyUrl, headers);
      if(r.ok || (r.status >= 400 && r.status < 500)) return r; // success or client error — stop trying
      // Server error from proxy itself — try next
    }catch(e){ lastErr = e; }
  }
  // Last resort: direct (works on local server)
  try{ return await doFetch(url, headers); }catch(e){ lastErr = e; }
  throw lastErr;
}
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    let reason = '';
    try{
      const rp = await fetchFavoriot(url, headers);
...
function refreshHealthDots(){
  ...
  list.forEach((d,i)=>{
    ...
    const a=document.getElementById(`dot-${i}`); if(a && a.style.background!==col) a.style.background=col;
    const b=document.getElementById(`vDot-${i}`); if(b && b.style.background!==col) b.style.background=col;
    const lbl=document.getElementById(`ol-${i}`);
    if(lbl){
      if(st.offline){
        lbl.textContent='Offline';
        lbl.className='badge ms-2 bg-danger-subtle text-danger-emphasis status-badge-transition';
      } else if(st.sleeping){
        lbl.innerHTML='Sleep <span class="zzz">💤</span>';
        lbl.className='badge ms-2 bg-warning-subtle text-warning-emphasis status-badge-transition';
      } else if(st.enoughBeacons){
        lbl.textContent='Online';
        lbl.className='badge ms-2 bg-success-subtle text-success-emphasis status-badge-transition';
      } else {
...
// CORS proxy dropdown logic
(function(){
  const sel = document.getElementById('corsProxySelect');
  const inp = document.getElementById('corsProxy');
  const KNOWN = ['','https://api.allorigins.win/raw?url=','https://api.cors.lol/?url=','https://proxy.corsfix.com/?','https://cors.isomorphic-git.org/'];
  function syncDropdown(proxyVal){
    if(KNOWN.includes(proxyVal)){ sel.value = proxyVal; inp.classList.add('d-none'); inp.value = proxyVal; }
    else { sel.value = 'custom'; inp.classList.remove('d-none'); inp.value = proxyVal; }
  }
  sel.addEventListener('change', ()=>{
    if(sel.value === 'custom'){ inp.classList.remove('d-none'); inp.value=''; inp.focus(); }
    else { inp.classList.add('d-none'); inp.value = sel.value; }
  });
  // Load saved proxy into dropdown on startup
  const saved = (CFG().proxy||'').trim();
  syncDropdown(saved);
  window._syncProxyDropdown = syncDropdown;
})();
...
document.getElementById('applyFavoriot').addEventListener('click', ()=>{
  ...
  const sel = document.getElementById('corsProxySelect');
  cfg.proxy = sel.value === 'custom' ? (document.getElementById('corsProxy').value||'').trim() : sel.value;
...
async function loadLatestScanToTester(){
  ...
  try{
    const r = await fetchFavoriot(url, headers);
...
function importAllFromObject(obj){
  ...
  if(window._syncProxyDropdown) window._syncProxyDropdown(cfg.proxy||'');
...
(function(){
      ...
      try{
        ...
        try{
          if (typeof streamsUrl==='function' && typeof headersFor==='function' && typeof fetchFavoriot==='function'){
            const url = streamsUrl(dev);
            const headers = headersFor();
            let resp = await fetchFavoriot(url, headers);
            let txt = await resp.text(); let j=null; try{ j=JSON.parse(txt); }catch{}
            if (!resp.ok){
              try{
                await new Promise(res=>setTimeout(res, 300));
                resp = await fetchFavoriot(url, headers);
  ...
  // -----------------------------------------------------------------------------------
  // 3) Badge tweak: now handled directly inside refreshHealthDots — no override needed
  // -----------------------------------------------------------------------------------
