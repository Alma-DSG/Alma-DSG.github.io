...
function refreshHealthDots(){
  ...
  list.forEach((d,i)=>{
    ...
    if(lbl){
      lbl.classList.add('status-badge-transition');
      let newText, newClass;
      if(st.offline){
        newText='Offline'; newClass='badge ms-2 bg-danger-subtle text-danger-emphasis status-badge-transition';
      } else if(st.sleeping){
        newText='Sleep 💤'; newClass='badge ms-2 bg-warning-subtle text-warning-emphasis status-badge-transition';
      } else {
        newText='Online'; newClass='badge ms-2 bg-success-subtle text-success-emphasis status-badge-transition';
      }
      // Only update DOM if something actually changed (prevents flicker/repaint)
      if(lbl.textContent !== newText || lbl.className !== newClass){
