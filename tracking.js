...
const lastServerTsByDev=new Map(); // stores the raw server timestamp for comparison
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    if(Number.isFinite(ts)){
      ...
      if(prevTs === undefined){
        // First fetch ever — use the real server timestamp so dead devices show correctly
        lastSeenByDev.set(normId, ts);
      } else if(ts > prevTs){
        // Server timestamp moved forward = genuinely new data arrived → show Online
        lastSeenByDev.set(normId, Date.now());
      }
      // If ts === prevTs, leave lastSeenByDev untouched so sleep/offline timer keeps counting
