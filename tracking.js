...
function computeStatus(normId){
  ...
  // Online window is based on when NEW data arrived (wall-clock), so it won't jump to Sleep
  // just because the stream timestamp is delayed/rounded/repeated.
  ...
  const online = !offline && (recentFetch || (Number.isFinite(delta) && delta <= SLEEP_GRACE_MS));
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    // Mark wall-clock time whenever NEW data arrives (robust against repeated/rounded timestamps)
    let packetKey = null;
