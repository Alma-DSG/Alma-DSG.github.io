...
// Persist lastSeenByDev across browser refreshes so status is correct immediately on reload
const LS_LASTSEEN = 'alps-lastseen-v1';
function saveLastSeen(){
  try{
    const o={};
    lastSeenByDev.forEach((v,k)=>{ if(Number.isFinite(v)) o[k]=v; });
    localStorage.setItem(LS_LASTSEEN, JSON.stringify(o));
  }catch(_){}
}
function loadLastSeen(){
  try{
    const o=JSON.parse(localStorage.getItem(LS_LASTSEEN)||'{}');
    Object.entries(o).forEach(([k,v])=>{ if(Number.isFinite(v)) lastSeenByDev.set(k,v); });
  }catch(_){}
}
loadLastSeen();
...
async function fetchOneDeviceLatest(dev, idx){
  ...
  try{
    ...
    if(Number.isFinite(ts)){
      const prev = lastSeenByDev.get(normId);
      // Only reset clock to now when the server timestamp is newer than before = new data arrived.
      // If timestamp is same or older, leave lastSeenByDev untouched so sleep timer is accurate.
