...
function trilatLM(beacons, initXY){
  ...
  // Weight each beacon by 1/d^2 — closer beacons are more reliable for RSSI ranging
  function getWeight(d){ return 1 / Math.max(0.1, d * d); }

  // Run one LM pass from a given starting point, returns {x, y, cost}
  function lmRun(sx, sy){
    let x=sx, y=sy;
    let lambda=1e-2;
    const maxIter=200, eps=1e-7;
    ...
    for(let it=0; it<maxIter; it++){
      ...
      for(const b of beacons){
        ...
        const ri = r - b.d;
        const w = getWeight(b.d);
        const jx = w*dx/r, jy = w*dy/r;
        const wri = w*ri;
      ...
      for(const b of beacons){
        ...
        const ri=r-b.d; const w=getWeight(b.d);
        newCost+=(w*ri)*(w*ri);
      }
  ...
  // Build candidate starting points: centroid, each beacon position, midpoints between pairs
  const cx = beacons.reduce((s,b)=>s+b.x,0)/beacons.length;
  const cy = beacons.reduce((s,b)=>s+b.y,0)/beacons.length;
  const starts = [ [cx, cy] ];
  if(initXY) starts.push(initXY);
  for(const b of beacons) starts.push([b.x, b.y]);
  for(let i=0;i<beacons.length;i++) for(let j=i+1;j<beacons.length;j++){
    starts.push([(beacons[i].x+beacons[j].x)/2, (beacons[i].y+beacons[j].y)/2]);
  }

  // Run LM from each start, keep best result
  let best=null;
  for(const [sx,sy] of starts){
    const res=lmRun(sx,sy);
